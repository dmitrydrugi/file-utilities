...
if (!$Confirmation -eq "" -And $Confirmation.ToUpper() -eq "Y") {
    try {
        ...
        ForEach ($File in $FileList) {
            ...
            Write-Progress -Activity "Changing of files encoding in progress" -Status "$PercentComplete% Complete:" `
            ...
            try {
                ...
                Write-Error -Message ("Error occured during processing of " + $File.FullName +
                        " file, Error: " + $_.Exception.Message) -Category ParserError -ErrorId "E-001"
            }
        }
    }
